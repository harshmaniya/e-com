...
const Navbar = () => {
    ...
    const [isUserLogin, setIsUserLogin] = useState(localStorage.getItem('accessToken') || null);
    ...
    return (
        <div className="bg-white sticky top-0 left-0 z-50 shadow-md">
            <nav className="flex justify-between items-center container mx-auto py-4 px-34">
                <div>
                ...
                <ul className='flex text-md text-black '>
                    ...
                    <li className={`mr-10 text-lg hover:border-b tracking-widest ${pathname.includes('adout') ? 'border-b' : ''}`}><Link href="/about">About</Link></li>
                    <li className={`mr-10 text-lg hover:border-b tracking-widest ${pathname.includes('products') ? 'border-b' : ''}`}><Link href="/products" className='pb-1'>Product</Link></li>
                </ul>
                <ul className='flex items-center text-2xl font-medium text-black'>                    
                    ...
                    <li className="relative px-2">
                        <span >
                            ...
                            {data?.getCart?.products?.length > 0 ? (
                                <>
                                    <span className="absolute -right-1 h-5 w-5 flex justify-center items-center text-white -top-2 rounded-full py-1 px-2 bg-[#9b6041] text-xs">
                                        {data.getCart.products.reduce((total, item) => total + item.qty, 0)}
                                    </span>
                                </>
                            ) : (
                                <span className="absolute -right-1 h-5 w-5 flex justify-center items-center text-white -top-2 rounded-full py-1 px-2 bg-[#9b6041] text-xs">0</span>
                            )}
                        </span>
                    </li>
