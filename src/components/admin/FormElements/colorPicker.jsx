...
const ColorPicker = ({ colors, setColors, handleAddColors }) => {
    ...
    const [IsColorExist, { loading: colorLoading }] = useMutation(IS_COLOR_EXIST);
                        ...
                        <div className='flex '>
                            <div className='w-1/4'>
                                ...
                                <button onClick={addColor} className="flex w-36 mt-6 my-3 justify-center rounded bg-primary p-3 font-medium text-gray hover:bg-opacity-90">
                                    Add {colors.length == 0 && colorLoading && <span>loading...</span>}
                                </button>
                            </div>
                                    ...
                                    <div key={index} className="px-3">
                                        <div className="">
                                            <div className="w-10 mx-auto h-10 relative bg-white rounded-full p-2"
                                                ...
                                                <svg onClick={() => removeColor(index)} className="w-6 h-6 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-black opacity-0 transition-all duration-200 ease-in-out hover:cursor-pointer hover:opacity-100" viewBox="0 0 20 20" fill="currentColor">
